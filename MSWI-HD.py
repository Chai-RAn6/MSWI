...
def load_cerna_data(
    ...
    for line in tqdm(lines, desc="Reading edges"):
        ...
        j = ndB - 1
        lay_idx = layID - 1
        adjacency_list[lay_idx][i, j] = w
        adjacency_list[lay_idx][j, i] = w  # 无向
...
def symmetric_kl_divergence(X, Y):
    X = np.array(X) + 1e-15
    Y = np.array(Y) + 1e-15
    return 0.5 * np.sum(X * np.log(X / Y)) + 0.5 * np.sum(Y * np.log(Y / X))

def hellinger_distance(p, q):
    p = np.array(p) + 1e-15  # 防止除 0
    q = np.array(q) + 1e-15
...
def compute_extra_layer_info_for_node(i, adjacency_list):
    ...
    for alpha in range(n_layers):
        ...
        for beta in range(n_layers):
            ...
            js_val = hellinger_distance(V_alpha, V_beta)
