library("igraph")
folder <- "data/Celegans_Multiplex_Genetic/"

edge.file <- paste(folder,"Dataset/celegans_genetic_multiplex.edges",sep="")
edge.list <- as.matrix(read.table(edge.file))

node.file <- paste(folder,"Dataset/celegans_genetic_nodes.txt",sep="")
node.list <- as.matrix(read.table(node.file, header=TRUE))

layer.file <- paste(folder,"Dataset/celegans_genetic_layers.txt",sep="")
...
#	g <- simplify(graph=g, remove.multiple=TRUE)
	...
	cat("D:\\\\Celegans_Multiplex_Genetic\\\\MuxViz\\\\",layer.name,".edgelist;",layer.name,";D:\\\\Celegans_Multiplex_Genetic\\\\MuxViz\\\\",layer.name,".nodes\n",sep="")
}

# record as R object
print(Celegans)
data.file <- paste(folder,"Celegans.Rdata",sep="")
save(Celegans, file=data.file)
